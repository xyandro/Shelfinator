		...
		void DotStar::RunThread(const char* device, int firstLight, int lightCount)
		{
			try
			{
				...
				if ((xfer[0].len + xfer[1].len + xfer[2].len) > GetBufSiz())
					throw "spidev.bufsiz too small; add spidev.bufsiz=8192 to /boot/cmdline.txt\n";
			...
			{
				fprintf(stderr, "DotStar::RunThread failed: %s\n", str);
				throw;
			}
			catch (...)
			{
				fprintf(stderr, "DotStar::RunThread failed\n");
				throw;
			}
		}		
